...
const TestUniswapLiquidity = artifacts.require("LiquidityExamples");

contract("LiquidityExamples", (accounts) => {
  ...
  it("add liquidity and remove liquidity", async () => {
    let tx = await contract.mintNewPosition({from: CALLER,});
    console.log("=== add liquidity ===");
    for (const log of tx.logs) {
      console.log(`${log.args.message} ${log.args.val}`);
    }

    tx = await contract.decreaseLiquidityInHalf(tokenA.address, tokenB.address, {
      from: CALLER,
    });
